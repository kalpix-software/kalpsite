...
//   3. Built-in default (production / SSL on).
//
// The default is PRODUCTION on purpose. Every real run sets the env var, so
// reaching the fallback means the config is already wrong — and of the two ways
// to be wrong, pointing at production fails loudly (a visibly wrong environment)
// while pointing anywhere else fails as an unexplained connection error. The
// previous default, api.kalpixgames.com, no longer resolves at all.
//
...
const ENV_HOST = process.env.NEXT_PUBLIC_KALPIX_HOST ?? 'api.kalpixsoftware.com';
