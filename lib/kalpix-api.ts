...
/**
 * Call an UNAUTHENTICATED backend RPC (no Bearer token). Only for the small set
 * of public endpoints the backend allows without a session — currently the
 * web account-deletion flow. Do not use for anything that needs a user.
 */
export async function publicRpc(rpcId: string, payload: string): Promise<unknown> {
  const base = (API_URL || '').replace(/\/$/, '');
  if (!base) {
    throw new Error('KALPIX_API_URL is not set');
  }
  const res = await fetch(`${base}/api/v1/${rpcId}`, {
    method: 'POST',
