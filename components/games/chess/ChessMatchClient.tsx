...
export default function ChessMatchClient({ matchId }: { matchId: string }) {
  ...
  // Native back button (Plak webview host): step back to the lobby within the
  ...
  /**
   * Per-viewer mute. Deliberately local and never sent: muting is "do not
   * distract me", not "silence this player for everyone" — which is what the
   * creator's switch is for.
   */
  const [chatMuted, setChatMuted] = useState(false);
  /**
   * Messages that arrived while the sheet was shut.
   *
   * Tracked by the seq of the last message the player has actually seen, not a
   * counter that is incremented on arrival: a reconnect replays the whole log,
   * and a counter would then announce a dozen "new" messages the player read
   * before the drop.
   */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (!auth?.token) {
          setError('No session token. Open this page from the Plak app.');
          return;
        }
  ...
  const handleOpCode = useCallback((op: number, data: Uint8Array) => {
    switch (op) {
      ...
      case ChatOp.Replay: {
        ...
        setChatDisabled(replay.disabled === true);
        // canToggle is the server's answer, replayed on every join so a
        // creator who reconnects mid-match regains the switch. Never inferred
        // from identity here.
        setCanToggleChat(replay.canToggle === true);
        if (replay.quickChatPresets) setQuickPresets(replay.quickChatPresets);
        return;
      }
      case ChatOp.SetDisabled: {
        setChatDisabled(decodeChat<ChatDisabledState>(data).disabled === true);
        return;
      }
      case ChatOp.Error: {
  ...
  // Mark everything read whenever the sheet is open and a message lands, so
  // reading with it open never leaves a stale badge behind.
  useEffect(() => {
    if (!chatOpen || chat.length === 0) return;
    setSeenChatSeq(chat[chat.length - 1].seq);
  }, [chatOpen, chat]);

  const unreadChat = chat.filter((e) => e.seq > seenChatSeq).length;
  const lastChatPreview = (() => {
    const last = chat[chat.length - 1];
    if (!last) return '';
    if (last.kind === 'sticker') return 'Sticker';
    return chatEntryText(last);
  })();

  const sendChatSticker = useCallback((stickerId: string) => {
    // Only the id goes over the wire; the server resolves the art after
    // re-checking ownership, so a doctored client cannot send art it lacks.
    sessionRef.current?.send(ChatOp.Sticker, encodeChat({ stickerId }));
  }, []);

  const setChatDisabledRemote = useCallback((disabled: boolean) => {
    sessionRef.current?.send(ChatOp.SetDisabled, encodeChat({ disabled }));
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* Top: back + opponent clock + status.
          The back control is not a duplicate of the hardware button — iOS has
          no hardware button and the webview shows no browser chrome, so without
          it an iOS player can reach this screen and cannot leave. It runs the
          same SPA step the native bridge does, so both routes behave alike. */}
      ...
      <footer className="flex flex-col gap-2 px-3 pb-3">
        ...
        {/* Tapping this opens the sheet rather than typing inline — the same
            model as Tero. An inline composer on a one-screen playfield either
            steals board height or gets covered by the keyboard. */}
        <button
          onClick={() => setChatOpen(true)}
          className="mt-2 flex w-full items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-left text-sm text-white/45"
        >
          <MessageCircle className="h-4 w-4 shrink-0" />
          <span className="flex-1 truncate">
            {chatDisabled ? 'Chat is off for this match' : lastChatPreview || 'Message…'}
          </span>
      ...
      <ChatSheet
        ...
        onSendSticker={sendChatSticker}
        onSetDisabled={setChatDisabledRemote}
        quickPresets={quickPresets}
        http={clientRef.current?.http ?? null}
        muted={chatMuted}
      ...
      <div className="w-full max-w-xs rounded-lg bg-zinc-900 p-6 text-center shadow-2xl">
        ...
        {result && (
          <div className="mt-5 flex flex-col gap-2 text-sm">
            {/* Free-games conversion: no coins row — chess has no entry fee. The
                XP / level-up / rating rows below are the match-end reward beat. */}
            {result.xpAwarded > 0 && (
