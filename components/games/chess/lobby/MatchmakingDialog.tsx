  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // Free-games conversion (PROG Act 2025): chess has no entry fee, so every
        // ticket carries the same "free" tier token — keeping the query string
        // identical for all players — and always bot-fills on timeout, since
        // there is no paid table to keep holding out for a human on.
        const handle = findMatch(p.client.socket, {
          minCount: 2,
          maxCount: 2,
          query: `+properties.gameId:chess +properties.timeControl:${p.timeControl} +properties.rated:true +properties.tier:free`,
          stringProperties: {
            gameId: 'chess',
