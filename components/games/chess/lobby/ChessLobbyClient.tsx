...
export default function ChessLobbyClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (!session?.token) {
        setBootError('No session. Open this page from the Plak app.');
        return;
      }
  ...
  // Free-games conversion (PROG Act 2025): chess has no entry fee, so tapping a
  // queue goes straight into matchmaking (which bot-fills after a 5s
  // human-search window) — no table/stake picker step.
