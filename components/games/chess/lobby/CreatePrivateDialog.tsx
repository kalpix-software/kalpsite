...
function InvitePanel({
  ...
  const share = async () => {
    ...
    // Inside the Plak webview, use the native share sheet via the
...
export function DialogShell({
  ...
  return (
    // Centred at every width, not a bottom sheet below sm. This renders inside
    // the Plak webview, which is phone-width but is not a mobile browser: there
    // is no URL bar or system gesture area at the bottom to sit above, so a
    // bottom sheet just pushed the content into the least visible part of the
    // screen. p-4 keeps the card off the edges on small screens.
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-4">
      <div
        ...
        style={{
          ...
          border: `1px solid ${lobbyTheme.divider}`,
          // Symmetric now that the card floats rather than rising from the
          // bottom edge; the old upward-only shadow read as detached mid-screen.
