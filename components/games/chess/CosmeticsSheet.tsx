...
export default function CosmeticsSheet(p: CosmeticsSheetProps) {
  ...
  return (
    // pt-[env(safe-area-inset-top)] is NOT redundant with the match root's copy
    // of it. An absolutely positioned child is laid out against its ancestor's
    // PADDING box, so inset-0 starts at the outer edge of that padding and the
    // header lands under the status bar / notch — where the close control was
    // drawn but could not be tapped. This re-applies the inset for the overlay.
    <div className="absolute inset-0 z-40 flex flex-col bg-black/80 backdrop-blur-sm pt-[env(safe-area-inset-top)]">
      <div className="flex items-center gap-3 px-4 pt-4">
        <BackButton onBack={p.onClose} label="Close customize" />
