...
/**
 * Upload a shop tile for a piece set.
 *
 * Deliberately named `preview.webp` inside the set's own folder rather than
 * beside the sprites with an arbitrary name: the renderer builds piece URLs as
 * baseUrl + "/" + FEN name, so any file whose stem collides with one of the
 * twelve would be fetched as a piece.
 */
async function uploadShopPreview(file: File, variantSlug: string): Promise<string> {
  const ct = contentTypeFor(file);
  const ext = ct === 'image/png' ? '.png' : ct === 'image/jpeg' ? '.jpg' : '.webp';
...
export default function ChessCosmeticUploader({ onUploaded }: { onUploaded?: () => void }) {
  ...
  // Shop name. Separate from the slug, which is baked into the item slug
  // (chess_pieces_<slug>) and the R2 path and so must stay stable.
  const [displayName, setDisplayName] = useState('');
  const [priceCoins, setPriceCoins] = useState(500);
  const [priceGems, setPriceGems] = useState(0);
  const [entries, setEntries] = useState<UploadEntry[]>([]);
  // Optional shop tile. Without one a piece set advertises itself with a bare
  // wK sprite on the shop grid, which shows one piece out of twelve and tells
  // a buyer almost nothing about the set they are paying for.
  const [shopPreview, setShopPreview] = useState<File | null>(null);
  ...
  async function uploadAll() {
    ...
    try {
      if (mode === 'pieces') {
        ...
        // An uploaded shop tile wins; otherwise fall back to the wK sprite so a
        // set uploaded without one still has something to show.
        let previewUrl = next.find((e) => stemOf(e.file.name) === 'wK')?.publicUrl ?? sample;
        if (shopPreview) {
          try {
            previewUrl = await uploadShopPreview(shopPreview, variantSlug);
          } catch (e) {
            setError(
              `Sprites uploaded, but the shop preview failed (${e instanceof Error ? e.message : 'error'}). ` +
                'Falling back to the wK sprite as the tile.',
            );
          }
        }
        if (!baseUrl) {
        ...
        const item = {
          ...
          name: displayName.trim() || `${capitalize(variantSlug)} Pieces`,
          description: `${displayName.trim() || capitalize(variantSlug)} piece set for Chess`,
          previewUrl,
  ...
  return (
    <div className="p-4 rounded-xl bg-slate-800 border border-slate-700 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            ...
            title="Becomes the item slug (chess_pieces_<slug>) and the R2 path. Lowercase and stable — changing it later orphans the uploaded art."
        ...
        <div>
          ...
          <input
            ...
            placeholder={`${capitalize(variant.trim().toLowerCase().replace(/[^a-z0-9_-]/g, '_')) || 'Auto'} Pieces`}
      ...
      {mode === 'pieces' && (
        <div className="rounded-lg border border-slate-700 bg-slate-900 p-3 space-y-1">
          <label className="block text-xs text-slate-400">
            Shop preview <span className="text-slate-500">(optional)</span>
          </label>
          <input
            type="file"
            accept="image/webp,image/png,image/jpeg"
            onChange={(e) => setShopPreview(e.target.files?.[0] ?? null)}
            className="w-full text-xs text-slate-300 file:mr-3 file:rounded file:border-0 file:bg-slate-700 file:px-3 file:py-1.5 file:text-xs file:text-slate-100"
          />
          <p className="text-[11px] text-slate-500">
            The tile shown in the shop and the in-match Customize sheet. Without one the set
            advertises itself with the bare <code>wK</code> sprite — one piece out of twelve.
            A shot of the whole set on a board sells it far better. Uploaded as{' '}
            <code>preview.webp</code> inside the set&rsquo;s folder, where it cannot collide with a
            piece name.
            {shopPreview && <> Selected: <strong>{shopPreview.name}</strong>.</>}
          </p>
        </div>
      )}
              ...
              <tr>
                ...
                {mode === 'pieces' && (
                  <th
                    ...
                    title="Average brightness of the sprite's visible pixels, 0 (black) to 255 (white). Checked because in a match each player supplies only their own colour half, so a set whose w* pieces are dark makes the board unreadable for BOTH players."
                  >
                    Brightness
                  </th>
                )}
                ...
                <tr key={i} className="border-t border-slate-700/50">
                  ...
                  {mode === 'pieces' && (
                    <td className="px-2 py-1 font-mono text-slate-400">
                      {e.luminance === undefined ? (
                        '—'
                      ) : (
                        <>
                          {Math.round(e.luminance)}
                          <span className="ml-1 text-slate-500">
                            {e.luminance < 110 ? 'dark' : e.luminance > 145 ? 'light' : 'mid'}
                          </span>
                        </>
                      )}
                    </td>
