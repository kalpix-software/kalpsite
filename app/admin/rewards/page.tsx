...
async function uploadDailyRewardImage(file: File, day: number): Promise<string> {
  ...
  // The per-day R2 key is deterministic (overwrites), so the stored URL has to
  // be cache-busted for the new art to show. admin_get_upload_url now stamps
  // publicUrl server-side for every item type — stamping again here would
  // produce a second '?' and break the URL.
  return publicUrl;
}
