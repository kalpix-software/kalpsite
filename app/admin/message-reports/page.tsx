...
export default function AdminMessageReportsPage() {
  ...
  const fetchReports = async (newCursor = '') => {
    ...
    try {
      const data = await callAdminRpc(
        ...
        JSON.stringify({
          channelId: filterChannelId.trim() || undefined,
  ...
  const updateStatus = async (report: MessageReport, newStatus: string) => {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-100 mb-1">Message Reports</h1>
      <p className="text-sm text-slate-400 mb-6">
        Review and manage reported messages. Filter by channel or status.
      </p>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <input
          ...
          placeholder="Filter by Channel ID (optional)"
      ...
      {reports.length > 0 ? (
                ...
                {reports.map((r) => {
                  ...
                  return (
                    <tr key={r.reportId} className="hover:bg-slate-800/50 transition">
                      ...
                      <td className="px-4 py-3">
                        <div className="text-slate-100 max-w-[250px] truncate" title={r.messageContent}>
                          {r.messageContent || '-'}
                        </div>
                        <div className="text-slate-500 text-xs">
                          {r.messageType && r.messageType !== 'text' && (
                            <span className="mr-2 px-1.5 py-0.5 rounded bg-slate-700 text-slate-400">{r.messageType}</span>
                          )}
                          <span className="font-mono truncate" title={r.messageId}>
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div className="text-slate-500 text-xs font-mono truncate max-w-[160px]" title={r.reportedId}>
                          {r.reportedId ? `${r.reportedId.slice(0, 12)}...` : '-'}
                        </div>
                      </td>
                      <td className="px-4 py-3">
        ...
        <div className="text-center py-12 text-slate-500">
          {reports.length === 0 && page === 1
            ? 'Click Search to load reports, or filter by channel ID / status.'
            : 'No reports found.'}
        </div>
      ) : null}
    </div>
