        ...
        categories: categories.map((cat) => ({
          ...
          subcategories: (cat.subcategories ?? []).map((sub) => ({
            ...
            options: (sub.options ?? []).map((opt) => {
              ...
              return {
                ...opt,
                // Carry the edited label through. Without this the table let you type a
                // label, showed it back, and then silently discarded it on save — the
                // spread above would keep the original humanize(optionId).
                // Falls back rather than writing an empty string: a cleared box should
                // mean "use the derived name", not "this option has no name".
                label: (row?.label ?? '').trim() || opt.label || humanize(opt.optionId),
...
export default function AdminAvatarsPage() {
  ...
  // ── Avatar identity: display name + thumbnail (the "DP" in the picker) ──
  // Both were previously bucket-only chores: the name came from whatever
  // sync_avatars derived from the slug, and the thumbnail had to be uploaded to
  // R2 by hand at exactly avatars/<slug>/thumbnail.webp.
  const [nameDraft, setNameDraft] = useState<Record<string, string>>({});
  const [savingName, setSavingName] = useState('');
  const [thumbUploading, setThumbUploading] = useState('');
  const [identityMsg, setIdentityMsg] = useState<{ id: string; text: string; error?: boolean } | null>(null);

  const saveAvatarName = async (a: AvatarListItem) => {
    const next = (nameDraft[a.avatarId] ?? a.avatarName).trim();
    if (!next || next === a.avatarName) return;
    setSavingName(a.avatarId);
    setIdentityMsg(null);
    try {
      await callRpc('avatar/admin_update_avatar', JSON.stringify({ avatarId: a.avatarId, avatarName: next }));
      setListAvatars((prev) => prev.map((x) => (x.avatarId === a.avatarId ? { ...x, avatarName: next } : x)));
      // Drop the draft so the Save button disappears and the field falls back to
      // the stored value — otherwise a trailing space would keep it showing.
      setNameDraft((p) => { const n = { ...p }; delete n[a.avatarId]; return n; });
      setIdentityMsg({ id: a.avatarId, text: 'Name saved' });
    } catch (e) {
      setIdentityMsg({ id: a.avatarId, text: e instanceof Error ? e.message : 'Save failed', error: true });
    } finally {
      setSavingName('');
    }
  };

  const uploadAvatarThumbnail = async (a: AvatarListItem, file: File) => {
    const allowed = ['image/webp', 'image/png', 'image/jpeg'];
    if (!allowed.includes(file.type)) {
      setIdentityMsg({ id: a.avatarId, text: 'Use WebP, PNG or JPEG.', error: true });
      return;
    }
    setThumbUploading(a.avatarId);
    setIdentityMsg(null);
    try {
      // Any filename is fine — the upload route rewrites the key to
      // avatars/<slug>/thumbnail.webp, so what the admin picked locally does
      // not have to match anything.
      const publicUrl = await uploadFileToR2(file, 'avatar_thumbnail', a.slug, '', '');
      // The key is deterministic, so the url never changes and caches keep
      // serving the old image. A cache-busting query makes the new one show up
      // in the panel immediately; the app gets it via the shorter browser TTL
      // the upload route sets on replaceable keys.
      await callRpc('avatar/admin_update_avatar', JSON.stringify({ avatarId: a.avatarId, previewUrl: publicUrl }));
      setListAvatars((prev) => prev.map((x) => (x.avatarId === a.avatarId ? { ...x, previewUrl: `${publicUrl}?v=${Date.now()}` } : x)));
      setIdentityMsg({ id: a.avatarId, text: 'Thumbnail updated' });
    } catch (e) {
      setIdentityMsg({ id: a.avatarId, text: e instanceof Error ? e.message : 'Upload failed', error: true });
    } finally {
      setThumbUploading('');
    }
  };
  ...
  const setPriceRow = (rowKey: string, field: 'label' | 'currencyType' | 'price' | 'salePrice' | 'purchaseLimit', value: string | number) => {
              ...
              <div className="overflow-x-auto rounded-lg border border-slate-600">
                <table className="w-full text-sm">
                  <thead className="bg-slate-700 text-slate-200">
                    <tr>
                      <th className="text-left px-3 py-2 w-20" title="Shown in the avatar picker">DP</th>
                      <th className="text-left px-3 py-2">Slug</th>
                      <th className="text-left px-3 py-2">Name (shown to players)</th>
                  ...
                  <tbody className="text-slate-300">
                    {listAvatars.length === 0 ? (
                      <tr><td colSpan={5} className="px-3 py-4 text-slate-500 text-center">No avatars in database. Upload Spine assets below.</td></tr>
                    ) : (
                      listAvatars.map((a) => (
                        <tr key={a.avatarId} className="border-t border-slate-600 hover:bg-slate-800/50">
                          <td className="px-3 py-2">
                            <label className="cursor-pointer block" title="Click to replace. Any filename works.">
                              {a.previewUrl ? (
                                /* eslint-disable-next-line @next/next/no-img-element */
                                <img src={a.previewUrl} alt="" className="w-11 h-11 rounded-lg object-cover border border-slate-600 hover:border-indigo-500" />
                              ) : (
                                <span className="w-11 h-11 rounded-lg border border-dashed border-slate-600 flex items-center justify-center text-[10px] text-slate-500 hover:border-indigo-500">
                                  {thumbUploading === a.avatarId ? '…' : 'Add'}
                                </span>
                              )}
                              <input
                                type="file"
                                ...
                                className="hidden"
                                disabled={thumbUploading === a.avatarId}
                                onChange={(e) => { const f = e.target.files?.[0]; if (f) void uploadAvatarThumbnail(a, f); e.target.value = ''; }}
                              />
                            </label>
                          </td>
                          <td className="px-3 py-2 font-mono text-xs">{a.slug}</td>
                          <td className="px-3 py-2">
                            {/* Explicit confirm rather than save-on-blur: this is the
                                name every player sees, and clicking away from a
                                half-typed name should not publish it. The button
                                only appears once the text differs from what is
                                stored, so it doubles as the unsaved-changes marker. */}
                            <div className="flex items-center gap-2">
                              <input
                                value={nameDraft[a.avatarId] ?? a.avatarName}
                                onChange={(e) => setNameDraft((p) => ({ ...p, [a.avatarId]: e.target.value }))}
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter') void saveAvatarName(a);
                                  if (e.key === 'Escape') setNameDraft((p) => { const n = { ...p }; delete n[a.avatarId]; return n; });
                                }}
                                disabled={savingName === a.avatarId}
                                title="What players see. Press Enter or click Save to apply; Escape to discard. The slug stays fixed — it keys the R2 paths."
                                className="w-44 px-2 py-1 rounded bg-slate-900 border border-slate-600 text-slate-100 text-sm"
                              />
                              {(nameDraft[a.avatarId] ?? a.avatarName).trim() !== a.avatarName && (
                                <>
                                  <button
                                    type="button"
                                    onClick={() => void saveAvatarName(a)}
                                    disabled={savingName === a.avatarId || !(nameDraft[a.avatarId] ?? '').trim()}
                                    className="px-2.5 py-1 rounded bg-indigo-600 text-white text-xs font-medium hover:bg-indigo-500 disabled:opacity-50"
                                  >
                                    {savingName === a.avatarId ? 'Saving…' : 'Save'}
                                  </button>
                                  <button
                                    type="button"
                                    onClick={() => setNameDraft((p) => { const n = { ...p }; delete n[a.avatarId]; return n; })}
                                    disabled={savingName === a.avatarId}
                              ...
                              {identityMsg?.id === a.avatarId && (
                                <span className={`text-xs ${identityMsg.error ? 'text-red-400' : 'text-emerald-400'}`}>{identityMsg.text}</span>
                              )}
                            </div>
                          </td>
                          <td className="px-3 py-2">
                  ...
                  {priceRows.map((r) => (
                    <tr key={r.rowKey} className="border-t border-slate-600">
                      ...
                      <td className="px-2 py-1">
                        {/* Editable. The default is humanize(optionId), which is only ever as
                            good as the Spine skin name — "face_3" becomes "Face 3" and there
                            is no way to call it "Smoky Eyes" without renaming the rig. This is
                            the label players actually see, so it has to be writable here. */}
                        <input
                          type="text"
                          value={r.label}
                          onChange={(e) => setPriceRow(r.rowKey, 'label', e.target.value)}
                          placeholder={humanize(r.optionId)}
                          title="Shown to players in the avatar picker. Defaults to the Spine skin name."
                          className="w-40 px-1.5 py-0.5 rounded bg-slate-900 border border-slate-600 text-slate-100 placeholder:text-slate-500"
                        />
                      </td>
                      <td className="px-2 py-1">
